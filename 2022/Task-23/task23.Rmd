...
get_blocked_targets <- function(proposal) {
  ...
  # Nur Positionen, die Vorschläge haben
  idx <- which(!is.na(proposal))
  if (!length(idx)) return(integer(0L))
  ...
  # Zielkoordinaten vorbereiten
  tr <- r
  tc <- c

  # Richtungscodes als Char
  sel <- p == "N"; tr[sel] <- tr[sel] - 1L
  sel <- p == "S"; tr[sel] <- tr[sel] + 1L
  sel <- p == "W"; tc[sel] <- tc[sel] - 1L
  sel <- p == "E"; tc[sel] <- tc[sel] + 1L

  # Nur gültige Ziele behalten
  valid <- (tr >= 1L & tr <= nr & tc >= 1L & tc <= nc)
  if (!any(valid)) return(integer(0L))

  # lineare Zielindizes
  tlin <- (tc[valid] - 1L) * nr + tr[valid]

  # Kollision: Ziele, die mehrfach vorkommen
  cnt <- tabulate(tlin, nbins = nr * nc)
  bad_targets <- cnt[tlin] > 1L

  # Ursprungsindizes der blockierten Elfen zurückgeben
  idx[valid][bad_targets]
}
...
tic("CP Version")
move_elves_sparse(puzzle_data, Inf)
toc()
```
