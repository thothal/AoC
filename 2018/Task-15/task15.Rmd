...
fight <- function(arena = puzzle_data, attack = c(G = 3L, E = 3L),
  ...
  get_target <- function(start) {
    ...
    if (any(enemy_idx)) {
      ...
      unit_idx <- duplicated(
        rbind(units[, 1:2, drop = FALSE], nbs),
        fromLast = TRUE)[1:nrow(units)]
      ...
      if (unit[, "hp"] > 0L) {
        ...
        if (!is.null(next_target)) {
          ...
          if (units[next_target, "hp"] <= 0L) {
            ...
            units[next_target, 1:2] <- -1L
          }
        }
      }
    }
    units <- units[order(units[, 1L], units[, 2L]), , drop = FALSE]
    ## move dead unit "outside" of the grid => makes collision detection much easier
    ## as a dead unit won't occupy a valid place
