...
make_graph <- function(df) {
  ...
  V(G)$orig_color[V(G)$type == "dest"] <- "forestgreen"
...
empty_node$col + ## moves to the left
  ...
  cols - 1L + ## moves on the top row to the right
...
```{r create-solution, eval = !file.exists(here("2016", "Task-22", "node_walk.gif"))}
...
move_load <- function(G) {
  sink_idx <- V(G)$is_sink
  sink_col <- V(G)$col[sink_idx]
  sink_row <- V(G)$row[sink_idx]
  load_row <- sink_row
  load_col <- sink_col + 1L
  load_idx <- V(G)$col == load_col & V(G)$row == load_row
  GS <- vector("list", 5L)
  ## 1. Move load to the left
  V(G)$color[sink_idx] <- "firebrick"
  V(G)$color[load_idx] <- "steelblue"
  V(G)$is_sink[sink_idx] <- FALSE
  V(G)$is_sink[load_idx] <- TRUE
  GS[[1L]] <- G
  ## 2. Move Sink down
  G <- move_sink("D", G)
  GS[[2L]] <- G
...
move_to_dest <- function(G) {
  sink_idx <- V(G)$is_sink
  sink_col <- V(G)$col[sink_idx]
  sink_row <- V(G)$row[sink_idx]
  free_row <- sink_row
  free_col <- sink_col + 1L
  free_idx <- V(G)$col == free_col & V(G)$row == free_row
  V(G)$color[sink_idx] <- "firebrick"
  V(G)$color[free_idx] <- "steelblue"
  G
}

save_plot <- function(G, j, base_dir, redo) {
  fn <- sprintf("cluster_%03d.png", j)
  if (redo || !file.exists(file.path(base_dir, fn))) {
    cat("Saving Plot <", fn, ">...\n", sep = "")
    png(file.path(base_dir, fn), 
        width = 12, height = 8, units = "in", res = 96, bg = "transparent")
    plot(G, vertex.label = NA)
    dev.off()
  }
}

create_gif <- function(G, redo = FALSE) {
  j <- 1
  base_dir <- here("2016", "Task-22", "img")
  dims <- list(start_row = V(G)$row[V(G)$is_sink],
               start_col = V(G)$col[V(G)$is_sink],
               nrow = max(V(G)$row),
               ncol = max(V(G)$col))
  save_plot(G, j, base_dir, redo)
  j <- j +1
  for (i in 1:dims$start_col) {
    G <- move_sink("L", G)
    save_plot(G, j, base_dir, redo)
    j <- j + 1
  }
  
  for(i in 1:dims$start_row) {
    G <- move_sink("U", G)
    save_plot(G, j, base_dir, redo)
    j <- j + 1
  }
  
  for (i in 1:(dims$ncol - 1L)) {
    G <- move_sink("R", G)
    save_plot(G, j, base_dir, redo)
    j <- j + 1
  }
  ...
  gif <- list.files(base_dir, pattern = "\\.png$", full.names = TRUE) %>% 
    lapply(image_read) %>% 
    image_join() %>% 
    image_animate(fps = 2L)
  image_write(image = gif, path = here("2016", "Task-22", "node_walk.gif"))
  gif
}
...
```{r show-walk, echo = FALSE}
knitr::include_graphics(here("2016", "Task-22", "node_walk.gif"))
```
